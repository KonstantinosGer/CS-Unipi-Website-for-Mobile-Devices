...
<head>
    ...
    <title>Πρόσκληση εκδήλωσης ενδιαφέροντος για την πρόσληψη εκπαιδευτικού προσωπικού με σχέση εργασίας ιδιωτικού δικαίου ορισμένου χρόνου</title>
...
<div id="main">
    <table id="table" align="center" border="0" style="width: 100%;">
        <?php
        try {
            #Connect to database
            $connection = new PDO("pgsql:host=localhost;dbname=cs_unipi_database", "postgres", "polifolio68");
            #Query
            $sql = 'SELECT * from dioikhtikes_prakseis ORDER BY id desc';

            foreach ($connection->query($sql) as $row) {
                if ($row["id"]==1){
                    #Create new object
                    $obj = new Dioikhtikes_Prakseis();
                    $obj->setTitle($row["title"]);
                    $obj->setDescription($row["description"]);
                    $obj->setArticleDate($row["article_date"]);
                    $obj->setReferencePhp($row["reference_jsp"]);
                    ...
                    ?>
                    <tr>
                        <td class="td-date"><b style="background-color: #3b5998; color: #f2f2f2; font-size: 16px;"><?php echo $obj->getArticleDate()?></b></td>
                        <td class="td-title"><h4><?php echo $obj->getTitle()?></h4><p><?php echo $obj->getDescription()?></p><a href="prokirixi-pliroforikis-ada-27-03-2015.pdf" target="_blank">Πρόσκληση εκδήλωσης ενδιαφέροντος για την πρόσληψη εκπαιδευτικού προσωπικού με σχέση εργασίας ιδιωτικού δικαίου ορισμένου χρόνου</a></td>
                    </tr>
                    <?php
                }
            }
