...
<head>
    ...
    <title>Μεταδιδακτορικοί Ερευνητές</title>
    ...
    <style>
        ...
        .td-name-title, .td-surname-title, .td-email-title {
            padding-bottom: 1em;
            font-size: 17px;
        }

        .td-name, .td-surname, .td-email{
            padding-bottom: 2em;
            font-size: 14px;
        }
...
class Metadidaktorikoi_Ereunhtes {
    // Properties
    private $name;
    private $surname;
    private $email;
...
<div id="main">
    <h1>Μεταδιδακτορικοί Ερευνητές</h1>
    ...
    <table id="table" align="center" border="0">
        ...
        try {
            ...
            $connection = new PDO("pgsql:host=localhost;dbname=cs_unipi_database", "postgres", "polifolio68");
            #Query
            $sql = 'SELECT * from metadidaktorikoi_ereunhtes ORDER BY id desc';

            foreach ($connection->query($sql) as $row) {
                #Create new object
                $obj = new Metadidaktorikoi_Ereunhtes();
