...
<head>
    ...
    <title>Υποψήφιοι Διδάκτορες</title>
    ...
    <style>
        ...
        .td-name-title, .td-surname-title, .td-email-title, .td-phone-number-title {
            padding-bottom: 1em;
            font-size: 17px;
        }

        .td-name, .td-surname, .td-email, .td-phone-number{
            padding-bottom: 2em;
            font-size: 14px;
        }
...
<div id="main">
    <h1>Υποψήφιοι Διδάκτορες</h1>

    <table id="table" align="center" border="0">
        <tr>
            <td class="td-name-title"><b>Όνομα</b></td>
            <td class="td-surname-title"><b>Επώνυμο</b></td>
            <td class="td-email-title"><b>Email</b></td>
            <td class="td-phone-number-title"><b>Τηλέφωνο</b></td>
        </tr>
        <?php
        try {
            #Connect to database
            $connection = new PDO("pgsql:host=localhost;dbname=cs_unipi_database", "postgres", "polifolio68");
            #Query
            $sql = 'SELECT * from ypopshfioi_didaktores ORDER BY id desc';

            foreach ($connection->query($sql) as $row) {
                #Create new object
                $obj = new Ypopshfioi_Didaktores();
                $obj->setName($row["name"]);
                $obj->setSurname($row["surname"]);
                $obj->setEmail($row["email"]);
                $obj->setPhoneNumber($row["phone_number"]);

                ?>
                <tr>
                    <td class="td-name"><p><?php echo $obj->getName()?></p></td>
                    <td class="td-surname"><p><?php echo $obj->getSurname()?></p></td>
                    <td class="td-email"><p><?php echo $obj->getEmail()?></p></td>
                    <td class="td-phone-number"><p><?php echo $obj->getPhoneNumber()?></p></td>
                </tr>
                <?php
            }
