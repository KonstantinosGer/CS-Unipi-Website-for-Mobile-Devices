...
<div id="main">
    <h1>Ερευνητικά Έργα</h1>

    <table id="table2" align="center" border="0" style="width: 100%;">
        <tr>
            <td><p>Το Τμήμα Πληροφορικής έχει συμμετάσχει στα παρακάτω ερευνητικά έργα:</p></td>
        </tr>
    </table>
    ...
    <table id="table" align="center" border="0" style="width: 100%;">
        ...
        try {
            ...
            $connection = new PDO("pgsql:host=localhost;dbname=cs_unipi_database", "postgres", "polifolio68");
            #Query
            $sql = 'SELECT * from ereunhtika_erga ORDER BY id desc';

            foreach ($connection->query($sql) as $row) {
                #Create new object
                $obj = new Ereunhtika_Erga();
                $obj->setTitle($row["title"]);
                $obj->setDescription($row["description"]);

                ?>
                <tr>
                    <td class="td-title"><h3>•<?php echo $obj->getTitle()?></h3><p><?php echo $obj->getDescription()?></p></td>
                </tr>
                <?php
            }
